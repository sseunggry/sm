// window.open("popup.html", "popup", "width= 500, height= 860, top=50, left= 50");
...
/*
$(function(){
    let $designTxt = $('.designTxt');
        // $firstDesignTxt = $designTxt.find('p').first();
    
    function DesignTxtMove() {
        stopDsignTxt();
        startDsignTxt();

        // let $lastDesignTxt = 
        $designTxt.find('p').last().prependTo($designTxt);
        // let $firstDesignTxt = 
        $designTxt.find('p').first().appendTo($designTxt);
        // let $firstDesignTxt = $designTxt.find('p').first();
    }


    startDsignTxt(); // 자동 슬라이드 시작
    let theInterval;
  
    function DesignTxtMove() {
      theInterval = setInterval(DesignTxtMove, 5000); //자동 슬라이드 설정
    }
  
    function stopDsignTxt() { //자동 멈추기
      clearInterval(theInterval);
    }
    
    $('.designTxt').hover(function(){ //마우스 오버시 슬라이드 멈춤
        stopDsignTxt();
    }, function (){
        startDsignTxt();
    });
